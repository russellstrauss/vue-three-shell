...
import { OrbitControls } from '@/assets/vendors/three.js/examples/jsm/controls/OrbitControls.js';
...
var gfx = (function() {
	...
	return {
		...
		consoleLogColor: function(color) { // three.js Color
			var argArray = [];
			
			console.log('#' + color.getHexString())
			
			let markup = '<span style="background-color:#' + color.getHexString() + ';"> Hex #' + color.getHexString() + ';' + '' + ' RGB(' + color.r +', ' + color.g + ', ' + color.b + ') </span>';
			...
			if (arguments.length) {
				...
				var reResultArray;
				argArray.push(markup.replace(startTagRe, '%c').replace(endTagRe, '%c'));
				while (reResultArray = startTagRe.exec(markup)) {
